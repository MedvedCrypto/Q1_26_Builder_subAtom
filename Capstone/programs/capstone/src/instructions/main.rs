...
use anchor_spl::token_interface::{
    burn, close_account, mint_to, Burn, CloseAccount, MintTo, TransferChecked,
};
use anchor_spl::{
...
declare_id!("8gLxKWtussxyWpprs3dsQds3a8WshSsWyBj5hLtt2jTy");

#[program]
pub mod solvency {
    use anchor_spl::{token::Transfer, token_interface};
    ...
    pub fn buy_subscription(
        ...
        let (metadata_pda, _) = Metadata::find_pda(&ctx.accounts.nft_mint.key());
        ...
        user_sub.claimed_by_creator_amount = 0;
    ...
    pub fn close_subscription(ctx: Context<CloseSubscription>) -> Result<()> {
        ...
        let upfront = ((plan.upfront_percentage as u128 * user_sub.total_deposit_amount as u128)
            / 100) as u64;
        let remaining = user_sub.total_deposit_amount - upfront;
        let vested_linear = if elapsed >= plan.vesting_duration {
            remaining
        } else {
        ...
        let plan_seeds: &[&[u8]] = &[b"plan", plan.creator.as_ref(), &[plan.bump]];
        ...
        if unclaimed > 0 {
            anchor_spl::token::transfer(
                CpiContext::new_with_signer(
                    ...
                    &[&[b"plan", plan.creator.as_ref(), &[plan.bump]]],
                ),
                unclaimed,
            )?;
        }
...
pub struct BuySubscription<'info> {
    ...
    #[account(
        ...
        constraint = payment_mint.key() == plan.payment_mint @ ErrorCode::InvalidPaymentMint
    )]
    ...
    pub nft_ata: InterfaceAccount<'info, InterfaceTokenAccount>,
...
pub struct CloseSubscription<'info> {
    ...
    pub creator_token: InterfaceAccount<'info, InterfaceTokenAccount>,
    #[account(
        mut,
...
pub struct ClaimTokens<'info> {
    ...
    #[account(mut, token::mint = plan.payment_mint, token::authority = plan.creator)]
    pub creator_token: InterfaceAccount<'info, InterfaceTokenAccount>,
    #[account(mut, signer, constraint = plan.creator == creator.key())]
